...
/// 蓝牙状态
public enum HHCoreBuleToothState: Error, CustomStringConvertible{
    case unknown
    case resetting
    case unsupported
    case unauthorized
...
/// CoreBluetooth库封装的BleCentral类，使用block的方式简化了蓝牙启动，扫描及配对的回调流程
public class HHCoreBuleToothManger: NSObject{
    
    public static let sharedManager = HHCoreBuleToothManger()
    ...
    //初始化回调
    fileprivate var initCompletion: ((HHCoreBuleToothState)->Void)?
    fileprivate var initError: ((HHCoreBuleToothState)->Void)?
    //扫描回调
    fileprivate var scanCallback: (([[String: AnyObject]])->Void)?
    fileprivate var scanCompletion: (()->Void)?
    fileprivate var scanError: ((HHCoreBuleToothState)->Void)?
    //连接回调
    fileprivate var connectedBlock: (()->Void)?
    fileprivate var connectdUpdateBlock: ((CBCharacteristic)->Void)?
    fileprivate var connectdErrorBlock: ((HHCoreBuleToothState)->Void)?
    ...
    /// 检查Central设备的蓝牙状态
    ///
    /// - returns  返回蓝牙状态                 :HHCoreBuleToothState
    public func checkBleStatus() -> HHCoreBuleToothState{
    ...
    public func bleInit(completion: ((HHCoreBuleToothState)->Void)?, error: ((HHCoreBuleToothState)->Void)?){
    ...
    public func scan(services: [CBUUID]?,discoverBlock: @escaping ([[String: AnyObject]])->Void, completionBlock: @escaping ()->Void, errorBlock: @escaping (HHCoreBuleToothState)->Void){
    ...
    /// - parameter updateValue      :设备数据回调
    /// - parameter errorBlock       :连接失败的回调
    public func connect(peripheral: CBPeripheral, connected: @escaping ()->Void, updateValue: @escaping (CBCharacteristic)->Void, errorBlock:@escaping (HHCoreBuleToothState)->Void){
...
extension HHCoreBuleToothManger: CBCentralManagerDelegate{
    
    public func centralManagerDidUpdateState(_ central: CBCentralManager){
        switch central.state {
        case .poweredOn:
            print("ble: bluetooth is poweredOn.")
...
extension HHCoreBuleToothManger: CBPeripheralDelegate{
